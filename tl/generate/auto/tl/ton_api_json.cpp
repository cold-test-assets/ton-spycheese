...
Result<int32> tl_constructor_from_string(ton_api::adnl_Address *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"adnl.address.reverse", 662262406}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::dht_ReversePingResult *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"dht.clientNotFound", 756842095},
    {"dht.reversePingOk", 541077666}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::dht_config_Global *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"dht.config.global", -2066822649},
    {"dht.config.global_v2", 1768129575}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::overlay_Certificate *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"overlay.certificateV2", -1270899581},
...
Result<int32> tl_constructor_from_string(ton_api::overlay_CertificateId *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"overlay.certificateId", -1884397383},
    {"overlay.certificateIdV2", -59977049}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::storage_PriorityAction *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"storage.priorityAction.all", -31225536},
    {"storage.priorityAction.idx", -1794132184},
    {"storage.priorityAction.name", 19190208}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::storage_daemon_NewContractParams *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"storage.daemon.newContractParams", 1432029316},
    {"storage.daemon.newContractParamsAuto", -1351129171}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::storage_daemon_SetPriorityStatus *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"storage.daemon.prioritySet", -1226268713},
    {"storage.daemon.priorityPending", -2079759962}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::storage_db_TorrentShort *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"storage.db.torrent", -1389545574},
    {"storage.db.torrentV2", -1980958565}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::validatorSession_Config *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"validatorSession.configVersioned", 1076532995},
    {"validatorSession.configVersionedV2", -1451552337}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::Object *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"dht.pong", 1519054721},
    {"dht.requestReversePingCont", -609369851},
    {"dht.clientNotFound", 756842095},
    {"dht.reversePingOk", 541077666},
    ...
    {"engine.validator.config", -400613680},
    ...
    {"engine.validator.fullNodeConfig", 704557332},
    ...
    {"engine.validator.onePerfTimerStat", -1859935384},
    {"engine.validator.oneStat", -1533527315},
    {"engine.validator.overlayStats", -543098119},
    {"engine.validator.overlayStatsNode", -109960999},
    {"engine.validator.overlaysStats", -1677121921},
    {"engine.validator.perfTimerStats", 1607488795},
    {"engine.validator.perfTimerStatsByName", -2101686812},
    {"engine.validator.proposalVote", 2137401069},
    {"engine.validator.shardOutQueueSize", 266052701},
    ...
    {"http.response", -901208246},
    {"http.proxy.capabilities", 831679505},
    ...
    {"storage.torrentInfo", 349098222},
    {"storage.updateInit", -835460938},
    {"storage.updateHavePieces", 1006116937},
    {"storage.updateState", 95433909},
    {"storage.daemon.contractInfo", -680243447},
    {"storage.daemon.fileInfo", 1903680510},
    {"storage.daemon.filePiecesInfo", 55915843},
    {"storage.daemon.keyHash", -2051710244},
    {"storage.daemon.newContractMessage", -175526410},
    {"storage.daemon.newContractParams", 1432029316},
    {"storage.daemon.newContractParamsAuto", -1351129171},
    {"storage.daemon.peer", -1120645068},
    {"storage.daemon.peerList", -1512163307},
    {"storage.daemon.providerAddress", -2007058158},
    {"storage.daemon.providerConfig", 2108492436},
    {"storage.daemon.providerInfo", -412417747},
    {"storage.daemon.queryError", 79542980},
    {"storage.daemon.prioritySet", -1226268713},
    {"storage.daemon.priorityPending", -2079759962},
    {"storage.daemon.speedLimits", -21960423},
    {"storage.daemon.success", -1280381156},
    {"storage.daemon.torrent", 364055399},
    {"storage.daemon.torrentFull", 1604881447},
    {"storage.daemon.torrentList", 1327241282},
    {"storage.daemon.torrentMeta", -725737752},
    {"storage.daemon.torrentPiecesInfo", 152873873},
    {"storage.daemon.config", -255421257},
    {"storage.daemon.provider.params", -1401721337},
    {"storage.db.config", 1569738024},
    {"storage.provider.db.contractAddress", -497488643},
    {"storage.provider.db.contractList", -633805033},
    {"storage.db.piecesInDb", 102347836},
    {"storage.db.priorities", 959048526},
    {"storage.db.torrentList", 1508893569},
    {"storage.db.torrent", -1389545574},
    {"storage.db.torrentV2", -1980958565},
    {"storage.db.key.config", -577227769},
    {"storage.db.key.pieceInDb", -1005916723},
    {"storage.db.key.piecesInDb", -605500957},
    {"storage.db.key.priorities", -1242445203},
    {"storage.db.key.torrentList", -876156842},
    {"storage.db.key.torrentMeta", 1646501222},
    {"storage.db.key.torrent", -1182264785},
    {"storage.provider.db.microchunkTree", -1026945214},
    {"storage.provider.db.state", 27874803},
    {"storage.provider.db.storageContract", -290216142},
    {"storage.provider.db.key.contractList", 1435683910},
    {"storage.provider.db.key.microchunkTree", 697887353},
    {"storage.provider.db.key.providerConfig", -404223694},
    {"storage.provider.db.key.state", -199176286},
    {"storage.provider.db.key.storageContract", -866202850},
    {"tcp.authentificate", 1146858258},
    {"tcp.authentificationNonce", -480425290},
    {"tcp.authentificationComplete", -139616602},
    {"tcp.pong", -597034237},
    ...
    {"tonNode.preparedState", 928762733},
    {"tonNode.notFoundState", 842598993},
    {"tonNode.privateBlockOverlayId", -1493903262},
    ...
    {"validatorSession.catchainOptions", 1893878246},
    {"validatorSession.config", -1235092029},
    {"validatorSession.configNew", -139482724},
    {"validatorSession.configVersioned", 1076532995},
    {"validatorSession.configVersionedV2", -1451552337},
    ...
    {"validatorSession.stats", 1749899919},
    {"validatorSession.statsProducer", -1912887795},
    {"validatorSession.statsRound", -1557143706},
...
Result<int32> tl_constructor_from_string(ton_api::Function *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"dht.registerReverseConnection", 573357153},
    {"dht.requestReversePing", 194290698},
    {"dht.store", 882065938},
    ...
    {"engine.validator.getOverlaysStats", -52908850},
    {"engine.validator.getPerfTimerStats", -364709649},
    {"engine.validator.getShardOutQueueSize", 1537301584},
    {"engine.validator.getStats", 1389740817},
    {"engine.validator.getTime", -515850543},
    {"engine.validator.importCertificate", 1015201999},
    {"engine.validator.importPrivateKey", 360741575},
    {"engine.validator.importShardOverlayCertificate", 448989784},
    {"engine.validator.setExtMessagesBroadcastDisabled", -1970206246},
    {"engine.validator.setVerbosity", -1316856190},
    {"engine.validator.sign", 451549736},
    {"engine.validator.signShardOverlayCertificate", 1553415254},
    {"getTestObject", 197109379},
    {"http.getNextPayloadPart", -1871422196},
    {"http.proxy.getCapabilities", -613277815},
    ...
    {"storage.daemon.addByHash", -1254791010},
    {"storage.daemon.addByMeta", -1235675554},
    {"storage.daemon.closeStorageContract", -159929584},
    {"storage.daemon.createTorrent", -1650869205},
    {"storage.daemon.deployProvider", 2108811237},
    {"storage.daemon.getNewContractMessage", -374603884},
    {"storage.daemon.getProviderInfo", 859666907},
    {"storage.daemon.getProviderParams", -1832020367},
    {"storage.daemon.getSpeedLimits", -1699835777},
    {"storage.daemon.getTorrentFull", 1553612902},
    {"storage.daemon.getTorrentMeta", 1935486449},
    {"storage.daemon.getTorrentPeers", 297259161},
    {"storage.daemon.getTorrentPiecesInfo", -206784730},
    {"storage.daemon.getTorrents", 590740314},
    {"storage.daemon.importPrivateKey", 2147449850},
    {"storage.daemon.initProvider", 673143500},
    {"storage.daemon.loadFrom", -937435609},
    {"storage.daemon.removeStorageProvider", 1121689494},
    {"storage.daemon.removeTorrent", 175789147},
    {"storage.daemon.sendCoins", 21329702},
    {"storage.daemon.setActiveDownload", 1954176413},
    {"storage.daemon.setActiveUpload", 1001305755},
    {"storage.daemon.setFilePriorityAll", -1921342855},
    {"storage.daemon.setFilePriorityByIdx", 1134024347},
    {"storage.daemon.setFilePriorityByName", -567467319},
    {"storage.daemon.setProviderConfig", -1918060660},
    {"storage.daemon.setProviderParams", 1615801628},
    {"storage.daemon.setSpeedLimits", 732030954},
    {"storage.daemon.setVerbosity", 649903000},
    {"storage.daemon.withdraw", -37961743},
    {"storage.getPiece", -2139429280},
    {"storage.getTorrentInfo", -1849387478},
...
Status from_json(ton_api::adnl_address_reverse &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::dht_requestReversePingCont &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "target", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.target_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signature", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "client", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.client_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::dht_clientNotFound &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "nodes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.nodes_, std::move(value)));
...
Status from_json(ton_api::dht_reversePingOk &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::dht_config_global_v2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "static_nodes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.static_nodes_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "network_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.network_id_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_config &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "fullnodeconfig", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.fullnodeconfig_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "liteservers", JsonValue::Type::Null, true));
...
Status from_json(ton_api::engine_validator_fullNodeConfig &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_messages_broadcast_disabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_messages_broadcast_disabled_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_onePerfTimerStat &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "min", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.min_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "avg", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.avg_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_overlayStats &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay_id_full", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_id_full_, std::move(value)));
    }
  }
  {
  ...
  }
  {
    ...
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.scope_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "nodes", JsonValue::Type::Null, true));
...
Status from_json(ton_api::engine_validator_overlayStatsNode &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "ip_addr", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ip_addr_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "bdcst_errors", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.bdcst_errors_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "fec_bdcst_errors", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.fec_bdcst_errors_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "last_in_query", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_in_query_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "last_out_query", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_out_query_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "t_out_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.t_out_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "t_in_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.t_in_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "t_out_pckts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.t_out_pckts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "t_in_pckts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.t_in_pckts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_overlaysStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlays", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlays_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_perfTimerStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "stats", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.stats_, std::move(value)));
...
Status from_json(ton_api::engine_validator_perfTimerStatsByName &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
...
Status from_json(ton_api::engine_validator_shardOutQueueSize &to, JsonObject &from) {
  {
...
Status from_json(ton_api::http_response &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "no_payload", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.no_payload_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::http_proxy_capabilities &to, JsonObject &from) {
  {
...
Status from_json(ton_api::overlay_certificateV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "issued_by", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.issued_by_, std::move(value)));
...
Status from_json(ton_api::overlay_certificateIdV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_id_, std::move(value)));
...
Status from_json(ton_api::storage_priorityAction_all &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "priority", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.priority_, std::move(value)));
...
Status from_json(ton_api::storage_priorityAction_idx &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "idx", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.idx_, std::move(value)));
...
Status from_json(ton_api::storage_priorityAction_name &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.name_, std::move(value)));
...
Status from_json(ton_api::storage_torrentInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.data_, std::move(value)));
...
Status from_json(ton_api::storage_updateInit &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "have_pieces_offset", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.have_pieces_offset_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "state", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_daemon_contractInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "torrent", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.torrent_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "created_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.created_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "file_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.file_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "downloaded_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.downloaded_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "rate", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.rate_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_span", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_span_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "client_balance", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.client_balance_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "contract_balance", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contract_balance_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_fileInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.name_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_filePiecesInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.name_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "range_l", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.range_l_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "range_r", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.range_r_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_keyHash &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "key_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.key_hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_newContractMessage &to, JsonObject &from) {
  {
...
Status from_json(ton_api::storage_daemon_newContractParams &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "rate", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.rate_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_newContractParamsAuto &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "provider_address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.provider_address_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_peer &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "ip_str", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ip_str_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "download_speed", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.download_speed_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "upload_speed", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.upload_speed_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ready_parts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ready_parts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_peerList &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "total_parts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_parts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_providerAddress &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_providerConfig &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "max_contracts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_contracts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_total_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_total_size_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_providerInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "contracts_count", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contracts_count_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "contracts_total_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contracts_total_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "contracts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contracts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_queryError &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "message", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.message_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_prioritySet &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_priorityPending &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_speedLimits &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "download", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.download_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "upload", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.upload_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_success &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_torrent &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "files_count", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.files_count_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "included_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.included_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dir_name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dir_name_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "downloaded_size", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "added_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.added_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "root_dir", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.root_dir_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "active_download", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.active_download_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "active_upload", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.active_upload_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "completed", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.completed_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "download_speed", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "fatal_error", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.fatal_error_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_torrentFull &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "torrent", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.torrent_, std::move(value)));
  ...
  }
  {
    ...
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.files_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_torrentList &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "torrents", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.torrents_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_torrentMeta &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "meta", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.meta_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_torrentPiecesInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "total_pieces", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_pieces_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "piece_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.piece_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "range_l", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "piece_ready_bitset", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.piece_ready_bitset_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "files", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_daemon_config &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "server_key", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.server_key_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cli_key_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cli_key_hash_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "provider_address", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "dht_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dht_id_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_provider_params &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "accept_new_contracts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.accept_new_contracts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "rate_per_mb_day", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.rate_per_mb_day_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_span", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "minimal_file_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.minimal_file_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "maximal_file_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.maximal_file_size_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_db_config &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "download_speed_limit", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.download_speed_limit_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "upload_speed_limit", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.upload_speed_limit_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_provider_db_contractAddress &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "wc", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.wc_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "addr", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_provider_db_contractList &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "contracts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contracts_, std::move(value)));
...
Status from_json(ton_api::storage_db_piecesInDb &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "pieces", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.pieces_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_db_priorities &to, JsonObject &from) {
  {
...
Status from_json(ton_api::storage_db_torrentList &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "torrents", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.torrents_, std::move(value)));
...
Status from_json(ton_api::storage_db_torrent &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "root_dir", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.root_dir_, std::move(value)));
...
Status from_json(ton_api::storage_db_torrentV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::storage_db_key_config &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_db_key_pieceInDb &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_db_key_piecesInDb &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_db_key_priorities &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_db_key_torrentList &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_db_key_torrentMeta &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_db_key_torrent &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_provider_db_microchunkTree &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.data_, std::move(value)));
...
Status from_json(ton_api::storage_provider_db_state &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "last_processed_lt", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_processed_lt_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_provider_db_storageContract &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "torrent_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.torrent_hash_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "microchunk_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.microchunk_hash_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "created_time", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_provider_db_key_contractList &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_provider_db_key_microchunkTree &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "wc", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.wc_, std::move(value)));
...
Status from_json(ton_api::storage_provider_db_key_providerConfig &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_provider_db_key_state &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_provider_db_key_storageContract &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "wc", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.wc_, std::move(value)));
...
Status from_json(ton_api::tonNode_privateBlockOverlayId &to, JsonObject &from) {
  {
...
Status from_json(ton_api::validatorSession_catchainOptions &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "idle_timeout", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.idle_timeout_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_deps", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_deps_, std::move(value)));
    }
  }
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "block_hash_covers_data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_hash_covers_data_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_block_height_ceoff", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_block_height_ceoff_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "debug_disable_db", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.debug_disable_db_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::validatorSession_configVersionedV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "catchain_opts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.catchain_opts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "round_candidates", JsonValue::Type::Null, true));
...
Status from_json(ton_api::validatorSession_stats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "timestamp", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.timestamp_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.self_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "creator", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.creator_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "total_validators", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_validators_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "total_weight", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_weight_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signatures", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "signatures_weight", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signatures_weight_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "approve_signatures", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "approve_signatures_weight", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.approve_signatures_weight_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "first_round", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.first_round_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "rounds", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.rounds_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorSession_statsProducer &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "block_status", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_status_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block_timestamp", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_timestamp_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorSession_statsRound &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "timestamp", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.timestamp_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "producers", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.producers_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::dht_registerReverseConnection &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "node", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_, std::move(value)));
...
Status from_json(ton_api::dht_requestReversePing &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "target", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.target_, std::move(value)));
...
Status from_json(ton_api::engine_validator_getOverlaysStats &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_getPerfTimerStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.name_, std::move(value)));
...
Status from_json(ton_api::engine_validator_getShardOutQueueSize &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "dest_wc", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dest_wc_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dest_shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dest_shard_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_importCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "signed_key", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signed_key_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cert", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cert_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_importShardOverlayCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "workchain", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.workchain_, std::move(value)));
...
Status from_json(ton_api::engine_validator_setExtMessagesBroadcastDisabled &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "disabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.disabled_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_signShardOverlayCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "workchain", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.workchain_, std::move(value)));
...
Status from_json(ton_api::http_proxy_getCapabilities &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "capabilities", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.capabilities_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_addByHash &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "start_download", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.start_download_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "allow_upload", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.allow_upload_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "priorities", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.priorities_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_daemon_addByMeta &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "meta", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.meta_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_closeStorageContract &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_createTorrent &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "copy_inside", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.copy_inside_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
...
Status from_json(ton_api::storage_daemon_deployProvider &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_getNewContractMessage &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "params", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.params_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_getProviderInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "with_balances", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.with_balances_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "with_contracts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.with_contracts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_getProviderParams &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_getSpeedLimits &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_getTorrentFull &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_getTorrentMeta &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_getTorrentPeers &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_getTorrentPiecesInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "max_pieces", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_pieces_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_getTorrents &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_importPrivateKey &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "key", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.key_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_initProvider &to, JsonObject &from) {
  {
...
Status from_json(ton_api::storage_daemon_loadFrom &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_removeStorageProvider &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_removeTorrent &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "remove_files", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.remove_files_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_sendCoins &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "address", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.address_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setActiveDownload &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "active", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.active_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::storage_daemon_setActiveUpload &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setFilePriorityAll &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setFilePriorityByIdx &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setFilePriorityByName &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.hash_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setProviderConfig &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "config", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.config_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setProviderParams &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "params", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.params_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setSpeedLimits &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_setVerbosity &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "verbosity", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.verbosity_, std::move(value)));
...
Status from_json(ton_api::storage_daemon_withdraw &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "contract", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.contract_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::storage_getTorrentInfo &to, JsonObject &from) {
  return Status::OK();
}
...
void to_json(JsonValueScope &jv, const ton_api::adnl_address_reverse &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.address.reverse");
}
...
void to_json(JsonValueScope &jv, const ton_api::dht_requestReversePingCont &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.requestReversePingCont");
  if (object.target_) {
    jo("target", ToJson(object.target_));
  }
  jo("signature", ToJson(JsonBytes{object.signature_}));
  jo("client", ToJson(object.client_));
}
void to_json(JsonValueScope &jv, const ton_api::dht_ReversePingResult &object) {
  ton_api::downcast_call(const_cast<ton_api::dht_ReversePingResult &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::dht_clientNotFound &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.clientNotFound");
  if (object.nodes_) {
    jo("nodes", ToJson(object.nodes_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::dht_reversePingOk &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.reversePingOk");
}
...
void to_json(JsonValueScope &jv, const ton_api::dht_config_Global &object) {
  ton_api::downcast_call(const_cast<ton_api::dht_config_Global &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::dht_config_global &object) {
  auto jo = jv.enter_object();
...
void to_json(JsonValueScope &jv, const ton_api::dht_config_global_v2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.config.global_v2");
  if (object.static_nodes_) {
    jo("static_nodes", ToJson(object.static_nodes_));
  }
  jo("k", ToJson(object.k_));
  jo("a", ToJson(object.a_));
  jo("network_id", ToJson(object.network_id_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_config &object) {
  ...
  if (object.fullnodeconfig_) {
    jo("fullnodeconfig", ToJson(object.fullnodeconfig_));
  }
  jo("liteservers", ToJson(object.liteservers_));
  jo("control", ToJson(object.control_));
  if (object.gc_) {
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_fullNodeConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.fullNodeConfig");
  jo("ext_messages_broadcast_disabled", ToJson(JsonBool{object.ext_messages_broadcast_disabled_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_onePerfTimerStat &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.onePerfTimerStat");
  jo("time", ToJson(object.time_));
  jo("min", ToJson(object.min_));
  jo("avg", ToJson(object.avg_));
  jo("max", ToJson(object.max_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlayStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.overlayStats");
  jo("overlay_id", ToJson(object.overlay_id_));
  if (object.overlay_id_full_) {
    jo("overlay_id_full", ToJson(object.overlay_id_full_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlayStatsNode &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.overlayStatsNode");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("ip_addr", ToJson(object.ip_addr_));
  jo("bdcst_errors", ToJson(object.bdcst_errors_));
  jo("fec_bdcst_errors", ToJson(object.fec_bdcst_errors_));
  jo("last_in_query", ToJson(object.last_in_query_));
  jo("last_out_query", ToJson(object.last_out_query_));
  jo("t_out_bytes", ToJson(object.t_out_bytes_));
  jo("t_in_bytes", ToJson(object.t_in_bytes_));
  jo("t_out_pckts", ToJson(object.t_out_pckts_));
  jo("t_in_pckts", ToJson(object.t_in_pckts_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlaysStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.overlaysStats");
  jo("overlays", ToJson(object.overlays_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_perfTimerStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.perfTimerStats");
  jo("stats", ToJson(object.stats_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_perfTimerStatsByName &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.perfTimerStatsByName");
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_shardOutQueueSize &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.shardOutQueueSize");
...
void to_json(JsonValueScope &jv, const ton_api::http_response &object) {
  ...
  jo("no_payload", ToJson(JsonBool{object.no_payload_}));
}
void to_json(JsonValueScope &jv, const ton_api::http_proxy_capabilities &object) {
  auto jo = jv.enter_object();
  jo("@type", "http.proxy.capabilities");
...
void to_json(JsonValueScope &jv, const ton_api::overlay_certificateV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.certificateV2");
  if (object.issued_by_) {
    jo("issued_by", ToJson(object.issued_by_));
  }
  jo("expire_at", ToJson(object.expire_at_));
...
void to_json(JsonValueScope &jv, const ton_api::overlay_CertificateId &object) {
  ton_api::downcast_call(const_cast<ton_api::overlay_CertificateId &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::overlay_certificateId &object) {
  auto jo = jv.enter_object();
...
void to_json(JsonValueScope &jv, const ton_api::overlay_certificateIdV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.certificateIdV2");
  jo("overlay_id", ToJson(object.overlay_id_));
  jo("node", ToJson(object.node_));
  jo("expire_at", ToJson(object.expire_at_));
  jo("max_size", ToJson(object.max_size_));
...
void to_json(JsonValueScope &jv, const ton_api::storage_PriorityAction &object) {
  ton_api::downcast_call(const_cast<ton_api::storage_PriorityAction &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::storage_priorityAction_all &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.priorityAction.all");
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_priorityAction_idx &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.priorityAction.idx");
  jo("idx", ToJson(JsonInt64{object.idx_}));
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_priorityAction_name &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.priorityAction.name");
  jo("name", ToJson(object.name_));
  jo("priority", ToJson(object.priority_));
}
...
void to_json(JsonValueScope &jv, const ton_api::storage_torrentInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.torrentInfo");
  jo("data", ToJson(JsonBytes{object.data_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::storage_updateInit &object) {
  ...
  jo("have_pieces_offset", ToJson(object.have_pieces_offset_));
  if (object.state_) {
    jo("state", ToJson(object.state_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_contractInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.contractInfo");
  jo("address", ToJson(object.address_));
  jo("state", ToJson(object.state_));
  jo("torrent", ToJson(object.torrent_));
  jo("created_time", ToJson(object.created_time_));
  jo("file_size", ToJson(JsonInt64{object.file_size_}));
  jo("downloaded_size", ToJson(JsonInt64{object.downloaded_size_}));
  jo("rate", ToJson(object.rate_));
  jo("max_span", ToJson(object.max_span_));
  jo("client_balance", ToJson(object.client_balance_));
  jo("contract_balance", ToJson(object.contract_balance_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_fileInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.fileInfo");
  jo("name", ToJson(object.name_));
  jo("size", ToJson(JsonInt64{object.size_}));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("priority", ToJson(object.priority_));
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_filePiecesInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.filePiecesInfo");
  jo("name", ToJson(object.name_));
  jo("range_l", ToJson(JsonInt64{object.range_l_}));
  jo("range_r", ToJson(JsonInt64{object.range_r_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_keyHash &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.keyHash");
  jo("key_hash", ToJson(object.key_hash_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_newContractMessage &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.newContractMessage");
  jo("body", ToJson(JsonBytes{object.body_}));
  jo("rate", ToJson(object.rate_));
  jo("max_span", ToJson(object.max_span_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_NewContractParams &object) {
  ton_api::downcast_call(const_cast<ton_api::storage_daemon_NewContractParams &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_newContractParams &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.newContractParams");
  jo("rate", ToJson(object.rate_));
  jo("max_span", ToJson(object.max_span_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_newContractParamsAuto &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.newContractParamsAuto");
  jo("provider_address", ToJson(object.provider_address_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_peer &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.peer");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("ip_str", ToJson(object.ip_str_));
  jo("download_speed", ToJson(object.download_speed_));
  jo("upload_speed", ToJson(object.upload_speed_));
  jo("ready_parts", ToJson(JsonInt64{object.ready_parts_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_peerList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.peerList");
  jo("peers", ToJson(object.peers_));
  jo("download_speed", ToJson(object.download_speed_));
  jo("upload_speed", ToJson(object.upload_speed_));
  jo("total_parts", ToJson(JsonInt64{object.total_parts_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_providerAddress &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.providerAddress");
  jo("address", ToJson(object.address_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_providerConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.providerConfig");
  jo("max_contracts", ToJson(object.max_contracts_));
  jo("max_total_size", ToJson(JsonInt64{object.max_total_size_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_providerInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.providerInfo");
  jo("address", ToJson(object.address_));
  jo("balance", ToJson(object.balance_));
  if (object.config_) {
    jo("config", ToJson(object.config_));
  }
  jo("contracts_count", ToJson(object.contracts_count_));
  jo("contracts_total_size", ToJson(JsonInt64{object.contracts_total_size_}));
  jo("contracts", ToJson(object.contracts_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_queryError &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.queryError");
  jo("message", ToJson(object.message_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_SetPriorityStatus &object) {
  ton_api::downcast_call(const_cast<ton_api::storage_daemon_SetPriorityStatus &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_prioritySet &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.prioritySet");
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_priorityPending &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.priorityPending");
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_speedLimits &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.speedLimits");
  jo("download", ToJson(object.download_));
  jo("upload", ToJson(object.upload_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_success &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.success");
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_torrent &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.torrent");
  jo("hash", ToJson(object.hash_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  ...
  jo("files_count", ToJson(JsonInt64{object.files_count_}));
  jo("included_size", ToJson(JsonInt64{object.included_size_}));
  jo("dir_name", ToJson(object.dir_name_));
  jo("downloaded_size", ToJson(JsonInt64{object.downloaded_size_}));
  jo("added_at", ToJson(object.added_at_));
  jo("root_dir", ToJson(object.root_dir_));
  jo("active_download", ToJson(JsonBool{object.active_download_}));
  jo("active_upload", ToJson(JsonBool{object.active_upload_}));
  jo("completed", ToJson(JsonBool{object.completed_}));
  jo("download_speed", ToJson(object.download_speed_));
  jo("upload_speed", ToJson(object.upload_speed_));
  jo("fatal_error", ToJson(object.fatal_error_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_torrentFull &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.torrentFull");
  if (object.torrent_) {
    jo("torrent", ToJson(object.torrent_));
  }
  jo("files", ToJson(object.files_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_torrentList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.torrentList");
  jo("torrents", ToJson(object.torrents_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_torrentMeta &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.torrentMeta");
  jo("meta", ToJson(JsonBytes{object.meta_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_torrentPiecesInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.torrentPiecesInfo");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("total_pieces", ToJson(JsonInt64{object.total_pieces_}));
  jo("piece_size", ToJson(object.piece_size_));
  jo("range_l", ToJson(JsonInt64{object.range_l_}));
  jo("range_r", ToJson(JsonInt64{object.range_r_}));
  jo("piece_ready_bitset", ToJson(JsonBytes{object.piece_ready_bitset_}));
  jo("files", ToJson(object.files_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_config &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.config");
  if (object.server_key_) {
    jo("server_key", ToJson(object.server_key_));
  }
  jo("cli_key_hash", ToJson(object.cli_key_hash_));
  jo("provider_address", ToJson(object.provider_address_));
  if (object.adnl_id_) {
    jo("adnl_id", ToJson(object.adnl_id_));
  }
  if (object.dht_id_) {
    jo("dht_id", ToJson(object.dht_id_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_provider_params &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.provider.params");
  jo("accept_new_contracts", ToJson(JsonBool{object.accept_new_contracts_}));
  jo("rate_per_mb_day", ToJson(object.rate_per_mb_day_));
  jo("max_span", ToJson(object.max_span_));
  jo("minimal_file_size", ToJson(JsonInt64{object.minimal_file_size_}));
  jo("maximal_file_size", ToJson(JsonInt64{object.maximal_file_size_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_config &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.config");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("download_speed_limit", ToJson(object.download_speed_limit_));
  jo("upload_speed_limit", ToJson(object.upload_speed_limit_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_contractAddress &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.contractAddress");
  jo("wc", ToJson(object.wc_));
  jo("addr", ToJson(object.addr_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_contractList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.contractList");
  jo("contracts", ToJson(object.contracts_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_piecesInDb &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.piecesInDb");
  jo("pieces", ToJson(JsonVectorInt64{object.pieces_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_priorities &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.priorities");
  jo("actions", ToJson(object.actions_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_torrentList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.torrentList");
  jo("torrents", ToJson(object.torrents_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_TorrentShort &object) {
  ton_api::downcast_call(const_cast<ton_api::storage_db_TorrentShort &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_torrent &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.torrent");
  jo("root_dir", ToJson(object.root_dir_));
  jo("active_download", ToJson(JsonBool{object.active_download_}));
  jo("active_upload", ToJson(JsonBool{object.active_upload_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_torrentV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.torrentV2");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("root_dir", ToJson(object.root_dir_));
...
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_config &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.config");
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_pieceInDb &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.pieceInDb");
  jo("hash", ToJson(object.hash_));
  jo("idx", ToJson(JsonInt64{object.idx_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_piecesInDb &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.piecesInDb");
  jo("hash", ToJson(object.hash_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_priorities &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.priorities");
  jo("hash", ToJson(object.hash_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_torrentList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.torrentList");
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_torrentMeta &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.torrentMeta");
  jo("hash", ToJson(object.hash_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_db_key_torrent &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.db.key.torrent");
  jo("hash", ToJson(object.hash_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_microchunkTree &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.microchunkTree");
  jo("data", ToJson(JsonBytes{object.data_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_state &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.state");
  jo("last_processed_lt", ToJson(JsonInt64{object.last_processed_lt_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_storageContract &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.storageContract");
  jo("torrent_hash", ToJson(object.torrent_hash_));
  jo("microchunk_hash", ToJson(object.microchunk_hash_));
  jo("created_time", ToJson(object.created_time_));
  jo("state", ToJson(object.state_));
  jo("file_size", ToJson(JsonInt64{object.file_size_}));
  jo("rate", ToJson(object.rate_));
...
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_key_contractList &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.key.contractList");
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_key_microchunkTree &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.key.microchunkTree");
  jo("wc", ToJson(object.wc_));
  jo("addr", ToJson(object.addr_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_key_providerConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.key.providerConfig");
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_key_state &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.key.state");
}
void to_json(JsonValueScope &jv, const ton_api::storage_provider_db_key_storageContract &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.provider.db.key.storageContract");
  jo("wc", ToJson(object.wc_));
  jo("addr", ToJson(object.addr_));
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_privateBlockOverlayId &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.privateBlockOverlayId");
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_catchainOptions &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.catchainOptions");
  jo("idle_timeout", ToJson(object.idle_timeout_));
  jo("max_deps", ToJson(object.max_deps_));
  jo("max_block_size", ToJson(object.max_block_size_));
  jo("block_hash_covers_data", ToJson(JsonBool{object.block_hash_covers_data_}));
  jo("max_block_height_ceoff", ToJson(object.max_block_height_ceoff_));
  jo("debug_disable_db", ToJson(JsonBool{object.debug_disable_db_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_configVersioned &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.configVersioned");
  jo("catchain_idle_timeout", ToJson(object.catchain_idle_timeout_));
  jo("catchain_max_deps", ToJson(object.catchain_max_deps_));
  jo("round_candidates", ToJson(object.round_candidates_));
  jo("next_candidate_delay", ToJson(object.next_candidate_delay_));
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_configVersionedV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.configVersionedV2");
  if (object.catchain_opts_) {
    jo("catchain_opts", ToJson(object.catchain_opts_));
  }
  jo("round_candidates", ToJson(object.round_candidates_));
  jo("next_candidate_delay", ToJson(object.next_candidate_delay_));
  jo("round_attempt_duration", ToJson(object.round_attempt_duration_));
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_stats &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.stats");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("timestamp", ToJson(JsonInt64{object.timestamp_}));
  jo("self", ToJson(object.self_));
  jo("creator", ToJson(object.creator_));
  jo("total_validators", ToJson(object.total_validators_));
  jo("total_weight", ToJson(JsonInt64{object.total_weight_}));
  jo("signatures", ToJson(object.signatures_));
  jo("signatures_weight", ToJson(JsonInt64{object.signatures_weight_}));
  jo("approve_signatures", ToJson(object.approve_signatures_));
  jo("approve_signatures_weight", ToJson(JsonInt64{object.approve_signatures_weight_}));
  jo("first_round", ToJson(object.first_round_));
  jo("rounds", ToJson(object.rounds_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorSession_statsProducer &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.statsProducer");
  jo("id", ToJson(object.id_));
  jo("block_status", ToJson(object.block_status_));
  jo("block_timestamp", ToJson(JsonInt64{object.block_timestamp_}));
}
void to_json(JsonValueScope &jv, const ton_api::validatorSession_statsRound &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.statsRound");
  jo("timestamp", ToJson(JsonInt64{object.timestamp_}));
  jo("producers", ToJson(object.producers_));
}
...
void to_json(JsonValueScope &jv, const ton_api::dht_registerReverseConnection &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.registerReverseConnection");
  if (object.node_) {
    jo("node", ToJson(object.node_));
  }
  jo("ttl", ToJson(object.ttl_));
...
void to_json(JsonValueScope &jv, const ton_api::dht_requestReversePing &object) {
  auto jo = jv.enter_object();
  jo("@type", "dht.requestReversePing");
  if (object.target_) {
    jo("target", ToJson(object.target_));
  }
  jo("signature", ToJson(JsonBytes{object.signature_}));
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getOverlaysStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getOverlaysStats");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getPerfTimerStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getPerfTimerStats");
  jo("name", ToJson(object.name_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getShardOutQueueSize &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getShardOutQueueSize");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  if (object.block_id_) {
  ...
  jo("dest_wc", ToJson(object.dest_wc_));
  jo("dest_shard", ToJson(JsonInt64{object.dest_shard_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_importCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.importCertificate");
  jo("overlay_id", ToJson(object.overlay_id_));
  if (object.local_id_) {
    jo("local_id", ToJson(object.local_id_));
  }
  if (object.signed_key_) {
    jo("signed_key", ToJson(object.signed_key_));
  }
  if (object.cert_) {
    jo("cert", ToJson(object.cert_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_importShardOverlayCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.importShardOverlayCertificate");
  jo("workchain", ToJson(object.workchain_));
  jo("shard", ToJson(JsonInt64{object.shard_}));
  if (object.signed_key_) {
    jo("signed_key", ToJson(object.signed_key_));
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_setExtMessagesBroadcastDisabled &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.setExtMessagesBroadcastDisabled");
  jo("disabled", ToJson(JsonBool{object.disabled_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_signShardOverlayCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.signShardOverlayCertificate");
  jo("workchain", ToJson(object.workchain_));
  jo("shard", ToJson(JsonInt64{object.shard_}));
  if (object.signed_key_) {
    jo("signed_key", ToJson(object.signed_key_));
...
void to_json(JsonValueScope &jv, const ton_api::http_proxy_getCapabilities &object) {
  auto jo = jv.enter_object();
  jo("@type", "http.proxy.getCapabilities");
  jo("capabilities", ToJson(JsonInt64{object.capabilities_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_addByHash &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.addByHash");
  jo("hash", ToJson(object.hash_));
  jo("root_dir", ToJson(object.root_dir_));
  jo("start_download", ToJson(JsonBool{object.start_download_}));
  jo("allow_upload", ToJson(JsonBool{object.allow_upload_}));
  jo("priorities", ToJson(object.priorities_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_addByMeta &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.addByMeta");
  jo("meta", ToJson(JsonBytes{object.meta_}));
  jo("root_dir", ToJson(object.root_dir_));
  jo("start_download", ToJson(JsonBool{object.start_download_}));
  jo("allow_upload", ToJson(JsonBool{object.allow_upload_}));
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_closeStorageContract &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.closeStorageContract");
  jo("address", ToJson(object.address_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_createTorrent &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.createTorrent");
  jo("path", ToJson(object.path_));
  jo("description", ToJson(object.description_));
  jo("allow_upload", ToJson(JsonBool{object.allow_upload_}));
  jo("copy_inside", ToJson(JsonBool{object.copy_inside_}));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_deployProvider &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.deployProvider");
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getNewContractMessage &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getNewContractMessage");
  jo("hash", ToJson(object.hash_));
  jo("query_id", ToJson(JsonInt64{object.query_id_}));
  if (object.params_) {
    jo("params", ToJson(object.params_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getProviderInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getProviderInfo");
  jo("with_balances", ToJson(JsonBool{object.with_balances_}));
  jo("with_contracts", ToJson(JsonBool{object.with_contracts_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getProviderParams &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getProviderParams");
  jo("address", ToJson(object.address_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getSpeedLimits &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getSpeedLimits");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getTorrentFull &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getTorrentFull");
  jo("hash", ToJson(object.hash_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getTorrentMeta &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getTorrentMeta");
  jo("hash", ToJson(object.hash_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getTorrentPeers &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getTorrentPeers");
  jo("hash", ToJson(object.hash_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getTorrentPiecesInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getTorrentPiecesInfo");
  jo("hash", ToJson(object.hash_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("offset", ToJson(JsonInt64{object.offset_}));
  jo("max_pieces", ToJson(JsonInt64{object.max_pieces_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_getTorrents &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.getTorrents");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_importPrivateKey &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.importPrivateKey");
  if (object.key_) {
    jo("key", ToJson(object.key_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_initProvider &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.initProvider");
  jo("account_address", ToJson(object.account_address_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_loadFrom &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.loadFrom");
  jo("hash", ToJson(object.hash_));
  jo("meta", ToJson(JsonBytes{object.meta_}));
  jo("path", ToJson(object.path_));
  if (object.flags_) {
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_removeStorageProvider &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.removeStorageProvider");
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_removeTorrent &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.removeTorrent");
  jo("hash", ToJson(object.hash_));
  jo("remove_files", ToJson(JsonBool{object.remove_files_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_sendCoins &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.sendCoins");
  jo("address", ToJson(object.address_));
  jo("amount", ToJson(object.amount_));
  jo("message", ToJson(object.message_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setActiveDownload &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setActiveDownload");
  jo("hash", ToJson(object.hash_));
  jo("active", ToJson(JsonBool{object.active_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setActiveUpload &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setActiveUpload");
  jo("hash", ToJson(object.hash_));
  jo("active", ToJson(JsonBool{object.active_}));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setFilePriorityAll &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setFilePriorityAll");
  jo("hash", ToJson(object.hash_));
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setFilePriorityByIdx &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setFilePriorityByIdx");
  jo("hash", ToJson(object.hash_));
  jo("idx", ToJson(JsonInt64{object.idx_}));
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setFilePriorityByName &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setFilePriorityByName");
  jo("hash", ToJson(object.hash_));
  jo("name", ToJson(object.name_));
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setProviderConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setProviderConfig");
  if (object.config_) {
    jo("config", ToJson(object.config_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setProviderParams &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setProviderParams");
  if (object.params_) {
    jo("params", ToJson(object.params_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setSpeedLimits &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setSpeedLimits");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("download", ToJson(object.download_));
...
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_setVerbosity &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.setVerbosity");
  jo("verbosity", ToJson(object.verbosity_));
}
void to_json(JsonValueScope &jv, const ton_api::storage_daemon_withdraw &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.daemon.withdraw");
  jo("contract", ToJson(object.contract_));
}
...
void to_json(JsonValueScope &jv, const ton_api::storage_getTorrentInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "storage.getTorrentInfo");
}
